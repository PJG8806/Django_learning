...
class BlogViewTestCase(TestCase):
    ...
    def test_blog_list(self):
        ...
        # context
        self.assertEqual(response.context.get('blog_list').count(), blog_list.count()) # 같은지 확인

    def test_blog_create_not_login(self):
        response = self.client.get(reverse('blog_create'))

        self.assertEqual(response.status_code, 302)
        self.assertEqual(response['Location'], settings.LOGIN_URL + '?next=/create/')# 같으면 로그인 요청이 떨어진거
