...
class NaverLoginRedirectView(RedirectView):
    def get_redirect_url(self, *args, **kwargs):
        ...
        # HTTP로 들어오면 HTTP, 127.0.0.1:8000 등의 들어온 주소대로 가져온다

        callback_url = domain + NAVER_CALLBACK_URL
        state = signing.dumps(NAVER_STATE) # 위조확인 부분 암호화
        ...
        return f'{NAVER_LOGIN_URL}?{urlencode(params)}' # redirect url로 변환 네이버에 로그인 요청
...
def naver_callback(request):
    ...
    access_token = get_naver_access_token(code, state) # 받아온 코드로 엑세스 토큰 받아온다

    profile_response = get_naver_profile(access_token) # 받아온 엑세스 토큰으로 네이버 프로필 가져오기 설정에 따라 가져오는 데이터는 다르다

    email = profile_response.get('email')

    user = User.objects.filter(email=email).first()

    if user: # 유저가 존재 하면 로그인
    ...
    return redirect( # 없으면 닉네임 입력 화면으로
...
def oauth_nickname(request):
    ...
    if form.is_valid():
        ...
        if oauth == 'naver':
            profile = get_naver_profile(access_token) # 받아오는 유저 정보
        else:
            profile = get_github_profile(access_token)

        email = profile.get('email')

        if User.objects.filter(email=email).exists(): # 엑세스 토큰을 가지고 접근 할시 존하는 이메일 경우 오류 출력
            raise Http404

        user.email = email

        user.is_active = True
        user.set_password(generate_random_password()) # 가짜 패스워드
        user.save()
...
def get_naver_access_token(code, state):
    ...
    response = requests.post(NAVER_TOKEN_URL, params=params)
...
def generate_random_password(length=10): # Django 5.0에서 make_random_password 삭제
...
class GithubLoginRedirectView(RedirectView):
    def get_redirect_url(self, *args, **kwargs):
        ...
        state = signing.dumps(GITHUB_STATE) # 위조확인 부분 암호화
        ...
        return f'{GITHUB_LOGIN_URL}?{urlencode(params)}' # redirect url로 변환 네이버에 로그인 요청
...
def github_callback(request):
    ...
    access_token = get_github_access_token(code, state) # 받아온 코드로 엑세스 토큰 받아온다
    if not access_token:
        raise Http404

    profile_response = get_github_profile(access_token) # 받아온 엑세스 토큰으로 네이버 프로필 가져오기 설정에 따라 가져오는 데이터는 다르다
...
def get_github_access_token(code, state):
    ...
    response = requests.post(GITHUB_TOKEN_URL, params=params)

    response_str = response.content.decode()

    response_dict = parse_qs(response_str)

    access_token = response_dict.get('access_token',[None])[0]
...
def get_github_profile(access_token):
    ...
    if not result.get('email'):
        result['email'] = f'{result["login"]}@id.github.com' # login이 id값
